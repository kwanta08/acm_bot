...
from config import INITIAL_TEAMS, SKILL_TAGS, config
...
class Members(commands.Cog):
    ...
    async def _sync_roles(self, guild: discord.Guild, member: discord.Member, user_id: str) -> None:
        ...
        primary_map = gconf.primary_team_role_ids
        secondary_map = gconf.secondary_team_role_ids
    ...
    async def register(self, interaction: discord.Interaction, user: discord.Member,
        ...
        await self.repo.upsert_member(
            guild_id, str(user.id), user.display_name, team.value if team else None)
        ...
        await self._sync_members_sheet(guild_id)
    ...
    async def assign_team(self, interaction: discord.Interaction, user: discord.Member,
        ...
        await self.repo.set_primary_team(guild_id, str(user.id), team.value)
        await interaction.followup.send(
    ...
    async def setup_member(
        ...
        if primary_team is not None:
            await self.repo.set_primary_team(guild_id, str(user.id), primary_team.value)
    ...
    async def set_channel(self, interaction: discord.Interaction,
        ...
        if guild_id is None:
            return
        await self.repo.upsert_team(guild_id, team.value, team.name, channel_id=str(channel.id))
        await interaction.followup.send(
    ...
    async def skill_add(self, interaction: discord.Interaction,
        ...
        await self.repo.add_skill(guild_id, str(target.id), skill.value)
        await interaction.followup.send(
    ...
    async def skill_remove(self, interaction: discord.Interaction,
        ...
        if guild_id is None:
            return
        target = user or interaction.user
        await self.repo.remove_skill(guild_id, str(target.id), skill.value)
        await interaction.followup.send(
    ...
    async def support(self, interaction: discord.Interaction,
        ...
        candidates = await self.repo.search_support(
            guild_id, team.value if team else None, skill.value if skill else None)
        cond = []
        if team:
    ...
    # ---------- 内部 ----------
    async def _sync_members_sheet(self, guild_id: int):
        sheets_cog = self.bot.get_cog("Sheets")
        if sheets_cog:
            try:
                await sheets_cog.sync_members(guild_id)
