...
class TodoistSetupModal(discord.ui.Modal, title="Todoist トークン登録"):
    ...
    async def on_error(self, interaction: discord.Interaction, error: Exception) -> None:
        # トークンを含めず、型名のみ記録する
        log.warning(
            "Todoist 登録 Modal でエラー (guild=%s): %s", self.guild_id, type(error).__name__
        )
        try:
...
class TodoistSetupView(discord.ui.View):
    ...
    async def open_modal(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id != self.owner_id:
            await interaction.response.send_message(
                embed=error_embed("このボタンはコマンドの実行者のみ使用できます。"), ephemeral=True
            )
            return
...
class TodoistAdmin(commands.Cog):
    ...
    async def register_token(
        ...
        *,
        ...
        """
        if not token:
            await interaction.followup.send(embed=error_embed("トークンが空です。"), ephemeral=True)
            return
        ...
        label = label_name or (existing["today_label_name"] if existing else DEFAULT_LABEL_NAME)
        project = (project_id or (existing["project_id"] if existing else None)) or None

        await self.repo.upsert(guild_id, encrypted, project, label, str(interaction.user.id))
    ...
    # ==================================================================
    @app_commands.command(
        name="todoist-status", description="このサーバーの Todoist 連携状態を表示します（管理者）。"
    )
    @app_commands.check(is_admin)
    async def todoist_status(self, interaction: discord.Interaction):
        ...
        embed.add_field(
            name="状態", value="✅ 有効" if cfg["enabled_flag"] else "⛔ 無効", inline=True
        )
        embed.add_field(
            name="プロジェクトID",
            value=f"`{cfg['project_id']}`" if cfg["project_id"] else "未設定",
            inline=True,
        )
        embed.add_field(name="ラベル名", value=f"`{cfg['today_label_name']}`", inline=True)
        embed.add_field(
            name="暗号鍵（ENCRYPTION_KEY）",
            value="✅ 設定済み" if enc_ok else "❌ 未設定/不正",
            inline=True,
        )
        embed.add_field(
            name="トークン復号テスト",
        ...
        embed.add_field(name="最終更新", value=cfg["updated_at"], inline=False)
    ...
    # ==================================================================
    @app_commands.command(
        name="todoist-remove", description="このサーバーの Todoist 設定を削除します（管理者）。"
    )
    @app_commands.check(is_admin)
    async def todoist_remove(self, interaction: discord.Interaction):
        ...
        if not deleted:
            await interaction.followup.send(
                embed=info_embed(
                    "Todoist 設定", "このサーバーには Todoist 設定が登録されていません。"
                ),
                ephemeral=True,
            )
            return
        await self.audit_repo.record(guild_id, str(interaction.user.id), "todoist.remove")
        await interaction.followup.send(
            embed=success_embed(
                "Todoist 設定を削除しました",
