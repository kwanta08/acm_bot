...
class LayerSessionRepository(BaseRepository):
    ...
    async def get_by_user(self, guild_id: int, user_id: str) -> dict[str, Any] | None:
        row = await self.db.fetchone(
            "SELECT * FROM layer_sessions WHERE guild_id = ? AND user_id = ?", (guild_id, user_id)
        )
        return dict(row) if row else None

    async def start(
        self, guild_id: int, user_id: str, keta: str, layer_num: str, started_at: str
    ) -> None:
    ...
    async def end(self, guild_id: int, user_id: str) -> None:
        """進行中セッションを削除する。"""
        await self.db.execute(
            "DELETE FROM layer_sessions WHERE guild_id = ? AND user_id = ?", (guild_id, user_id)
        )

    async def list_all(self, guild_id: int) -> list[dict[str, Any]]:
        rows = await self.db.fetchall(
            "SELECT * FROM layer_sessions WHERE guild_id = ? ORDER BY started_at", (guild_id,)
        )
