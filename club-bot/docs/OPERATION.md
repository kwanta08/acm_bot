...
| コマンド | 権限 | 説明 |
|---|---|---|
| `/progress` | L1 | 進捗をドリルダウン表示（機体 → パーツ → 部品 → …。各階層で横棒グラフ画像付き） |
| `/progress-setup <スプレッドシートID>` | L4 | 進捗管理シートを登録し、3シート（進捗管理・Todoist対応表・ダッシュボード）を初期化 |
| `/progress-sync` | L4 | Todoist 同期＋再集計を即時実行（通常は20分ごとに自動実行） |

機体製作の進捗を **Google Sheets を正本** として管理し、Discord から
ドリルダウンで確認できる機能です。深さに制限はなく、親子関係
（`ID` / `親ID` 列）だけで機体 → パーツ → 部品 → サブタスク…と
自由に階層化できます。

**進捗管理シートの列**（`/progress-setup` が自動作成します）
...
| 進捗率(手入力) | 葉ノードのみ有効（0.5 / 50% どちらでも可） | 人間 or Todoist同期 |
| 集計進捗率 | 葉=手入力値、親=子の平均を再帰計算 | bot（自動計算） |
| 進捗バー | SPARKLINE 数式（bot が空セルに1回だけ設置） | —（自動追従） |
| ソース | `manual` または `todoist` | bot |
...
2. Todoist対応表シートに「プロジェクト名 → 紐付け先ノードID」を1行ずつ記入
   例: 主翼班 | wing   ← 進捗管理シートの主翼パーツの ID
3. 以降20分ごとに自動同期:
...
- `ソース=manual` の行は同期処理が **絶対に上書きしません**（手動管理の
  パーツと Todoist 管理のパーツを同じツリーに混在できます）。
