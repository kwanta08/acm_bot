...
### Directus 管理（閲覧アカウント発行・任意）
| コマンド | 権限 | 説明 |
|---|---|---|
| `/directus-setup` | L4 | このサーバー用の Directus 閲覧アカウントを発行（引数なし。表示されるボタンからフォーム（Modal）を開いてメールアドレスを入力） |
| `/directus-status` | L4 | 発行状況を表示（メールアドレス・状態・発行日時） |
| `/directus-revoke` | L4 | 発行済みアカウントを失効（Directus 側で停止） |

DB を表形式で閲覧・編集できる UI（Directus）のアカウントを、サーバーごとに
発行するための任意連携です。発行されたアカウントで見えるのは
**そのサーバーのデータだけ**です（Directus の行レベル権限フィルタによる分離）。

- メールアドレスはコマンドの引数では受け取りません（オプション値は Discord の
  履歴に残るため）。Modal の入力値はチャンネル履歴に残りません。
- **パスワードは Bot が生成も保存もしません**。Directus の招待メールから
  本人が設定します（`guild_directus_access` テーブルに秘密情報は保存されません）。
- 1つのアカウントは1つのサーバーにしか紐付けられないため、同じメール
  アドレスを複数のサークルで使うことはできません。
- Bot の運用者が Directus を用意していない場合、コマンドはエラーにならず
  「未設定です」という案内を表示します。
- 発行・失効は監査ログに記録されます。
- セットアップ手順は [`DIRECTUS.md`](DIRECTUS.md) を参照してください。
...
- `/sheet_sync` は対象シートを毎回全件上書きします（手動編集は消えます）。
  閲覧・編集が主目的なら Directus（[`DIRECTUS.md`](DIRECTUS.md)）の利用を推奨します。
...
記録の閲覧・編集は Directus から行えます（[`DIRECTUS.md`](DIRECTUS.md)）。
...
- **記録の閲覧**: Directus を使うと DB を表形式で閲覧・検索できる（[`DIRECTUS.md`](DIRECTUS.md)）。
...
A. 廃止されました。記録の正本は SQLite/PostgreSQL で、閲覧・編集には Directus を使います。
...
し直してください。誤記録の行は Directus から削除できます。
