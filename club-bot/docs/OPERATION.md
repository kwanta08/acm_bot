...
- `/sheet_sync` は対象シートを毎回全件上書きします（手動編集は消えます）。
  閲覧・編集が主目的なら NocoDB（[`NOCODB.md`](NOCODB.md)）の利用を推奨します。
...
記録の閲覧・編集は NocoDB から行えます（[`NOCODB.md`](NOCODB.md)）。
...
- **記録の閲覧**: NocoDB を使うと DB を表形式で閲覧・検索できる（[`NOCODB.md`](NOCODB.md)）。
...
A. 廃止されました。記録の正本は SQLite で、閲覧・編集には NocoDB を使います。
...
し直してください。誤記録の行は NocoDB から削除できます。
