...
2. **【準備B】**（任意）Todoist の連携情報を準備する
3. **【STEP1】** さくらのVPSを契約して Ubuntu を用意する
4. **【STEP2】** VPSに接続して、初期設定（更新・作業ユーザー作成）
5. **【STEP3】** Bot のファイルを配置し、`.env` を設定する
6. **【STEP4】** 手動で起動して動作確認する
7. **【STEP4+】** Discord 上で `/setup` を実行し、通知チャンネル・ロールの設定と班の作成を行う
8. **【STEP5】** systemd で 24 時間常駐させる
9. **【STEP6】** バックアップと日常メンテ
...
> **`.env` に書いた場合は `.env` が優先されます。** `/setup` や `/set_role` で
> 班長ロールを全部外しても、`.env` の値が残っていれば権限は残ります
> （コマンドの結果にその旨が表示されます）。
> サーバーごとに設定を変えたい場合は `.env` から `LEADER_ROLE_IDS` を消し、
...
## B-2. 本番データベース（PostgreSQL・任意）

本番では記録の正本を **PostgreSQL** とします
（`deploy/docker-compose.postgres.yml` で PostgreSQL を起動し、
...
## 【STEP4+】Discord 上で `/setup` を実行して班を作成する

Bot が動いたら、Discord のサーバー上で**管理者が `/setup` を実行**します。

1. 設定状況の一覧（通知チャンネル・ログチャンネル・管理者ロール）が表示されます。
   未設定の項目はセレクトメニューから対話的に設定できます。
2. 「**班を一括作成**」ボタンを押し、班名をカンマ区切りで入力すると、
   班と対応する Discord ロールをまとめて作成できます
   （例: `設計班, 製造班, 広報班`）。
3. 班は**自動作成されません**。必ずこの手順か `/team-add` で作成してください。
   後からの追加・変更は `/team-add` `/team-remove` `/team-role` で行えます。
