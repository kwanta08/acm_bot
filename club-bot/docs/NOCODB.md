...
> ⚠️ **この構成はレガシーです。新規導入では [`DIRECTUS.md`](DIRECTUS.md) を参照してください。**
>
> NocoDB ではギルドごとの分離を「テーブルごとにフィルタビューを手動で複製する」
> （本書 4章）方法で実現していたため、参加サークルが増えるたびに運用者の
> 手作業が必要でした。Directus はユーザー単位の動的な権限フィルタを
> 一度設定するだけで分離が効き、各サークルの管理者が `/directus-setup` で
> 自分のアカウントを発行できます。
>
> 移行手順は [`DIRECTUS.md`](DIRECTUS.md) 7章にあります（業務 DB はそのまま使えます）。
> 本書は既存の NocoDB 運用を続ける場合の参照用として残しています。
> なお 3章（SQLite からの移行）・5章（バックアップ）・8章（Sheets からの移行）は
> Directus 構成でもそのまま通用します。
