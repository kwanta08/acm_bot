...
2. **班・技能タグの初期値は投入しない**（新規ギルドは空で開始し、
   管理者が `/team-add` `/skill-add` で登録する）
3. 権限があればロール（`幹部` / `Bot管理者`）と `#bot-log` チャンネルを作成し、
   ID を settings に保存（`set_if_absent` のみ。既存設定は上書きしない）。
   **班ロールは自動作成しない**（`/team-add` と `/team-role` で紐付ける）
4. 2重作成は **実効設定に ID があるか**（`EXEC_ROLE_ID` / `ADMIN_ROLE_ID` /
   `BOT_LOG_CHANNEL_ID`。環境変数フォールバックを含み、そのギルドに実在するかまで見る）と、
   **`AUTO_SETUP_COMPLETED_AT` マーカー**で防ぐ。マーカーは
   **全部揃ったときだけ**立て、立っていたらやり直さない
   （管理者が後から消した設定を復活させないため）。
   旧 `AUTO_SETUP_DONE` は**読まない** — 旧実装が権限不足で何も作れなかった
   ギルドにも立てており、これで早期 return すると「権限を付けて再招待」という
   復旧手順が永久に効かなくなるため
5. スラッシュコマンドはグローバル登録に統一し、そのギルドに残っている
   旧ギルド限定コマンドを1回だけ除去する（`_clear_legacy_guild_commands`）
6. 導入手順の案内（`/setup` / `/setup-status` / `/help`）を
   bot-log →（無ければ）`system_channel` →（無ければ）送信可能な最初の
   テキストチャンネル、の順で1箇所へ送る（`send_guild_notice`）

起動時（on_ready 初回）も参加中の全ギルドに 1〜5 を適用するため、
オフライン中に招待されていたギルドや、権限が後から付与されたギルドでも
セットアップが追いつく（**案内 6 は on_guild_join のときだけ**で、
再起動では送り直さない）。
