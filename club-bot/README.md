...
招待した瞬間にサーバー登録が走り、**導入手順の案内**（`/setup` `/setup-status`
`/help`）が1つのチャンネルへ投稿されます（`#bot-log` があればそこ、無ければ
システムメッセージ用チャンネル、それも無ければ Bot が書き込める最初のチャンネル）。

`幹部` / `Bot管理者` ロールと `#bot-log` チャンネルの自動作成は、
`ロールの管理` / `チャンネルの管理` を持っているときだけ行われます
（**上記の最小権限には含まれません**）。自動作成を使う場合は招待後に追加で付与してください。
無い場合はその部分がスキップされるだけで、自分で作成して `/setup` で指定すれば同じように動きます。
...
- 通知チャンネル・ログチャンネル・実行役（幹部）ロール・管理者ロールを
  セレクトメニューから設定（不足は `/setup-status` で確認できます）
...
| 機能 | 内容 |
|---|---|
| コマンドの入口 | `/help` でカテゴリからコマンドを探せます。`/setup-status` で初期設定の未完了項目を確認 |
| 日程調整 | 候補日時へのリアクション投票・自動締切・未回答者への催促DM（`/schedule`）。投票に使う絵文字はサーバーごとに変更可 |
| タスク管理 | 期限付きタスクの登録・班別通知・`/today` ラベル（`/task`）。**タスクの正本は Todoist** なので `/todoist-setup` の登録が必要です |
| 班・メンバー管理 | 班の作成・ロール紐付け・班をまたいだ支援候補の検索（`/team-*` `/member`） |
| 重量管理 | 機体の目標重量と実測重量をツリーで積み上げ、目標超過の大きい順に表示（`/weight`）。人力機は重量が成績に直結します |
| 大会からの逆算 | 大会日と節目の期限から必要ペースを逆算し、遅れているものを知らせます（`/milestone` `/countdown`）。遅延がある週だけ月曜に通知 |
| 年度替わり | 卒業者の仕分け・班長フラグの一括リセット・年度スナップショットの受け取り（`/season rollover`）。卒業者の記録は消しません |
| リマインド | 締切前の未回答催促・毎朝の期限タスク通知・期限超過の警告 |
| レポート | 週次サマリー（サークル名入り）・タスクCSV出力・出欠率・監査ログ（`/report`） |
| データの持ち出し・削除 | `/data export` で主要14テーブルを ZIP（CSV 群）に。`/data delete` で自分のサーバーのデータを自分で削除（`/data delete-cancel` で取り消し可） |
| エクスポート | 各表を CSV でダウンロード（Web ダッシュボード。Excel でもそのまま開けます） |
...
**運営者への連絡は不要です。** Botをサーバーからキックすれば動作が止まり、
**30日後にデータは自動削除されます**（誤操作に備えた猶予期間です。
その間に再招待すればデータはそのまま戻ります）。

すぐに消したい場合は、サーバー管理者が `/data delete` を実行してください
（確認のためサーバー名の入力が必要です）。消す前に `/data export` で
主要14テーブルを ZIP（CSV 群）として持ち出せます。`/data delete-cancel` で
削除の取り消しもできます。詳細は
[プライバシーポリシー](docs/PRIVACY.md) の「保存期間」を参照してください。
