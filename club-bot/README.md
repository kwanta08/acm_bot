...
| データの持ち出し・削除 | `/data export` で主要18テーブルを ZIP（CSV 群）に。`/data delete` で自分のサーバーのデータを自分で削除（`/data delete-cancel` で取り消し可） |
...
主要18テーブルを ZIP（CSV 群）として持ち出せます。`/data delete-cancel` で
削除の取り消しもできます。詳細は
