...
# ---------------------------------------------------------------------
async def import_tasks(db: Database, guild_id: int, rows: list[list], apply: bool) -> Stats:
    ...
    for i, row in enumerate(rows, start=2):
        ...
        try:
            ...
            if apply:
                ...
                skills = [
                    s.strip() for s in _cell(row, MEM_COLS["skills"]).split("、") if s.strip()
                ]
                # 技能タグをギルドのマスタに自動登録
                for skill in skills:
                    await db.execute(
...
async def main(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        all_stats.append(
            (
                f"tasks({tasks_name})",
                await import_tasks(db, guild_id, sheets["main"].get(tasks_name, []), args.apply),
            )
        )
        all_stats.append(
            (
