...
環境変数（移行時のみ使用）:
    ...
    SHEET_ATTENDANCE / SHEET_MEMBERS  各シート名（既定 attendance/members）
...
MEM_COLS = {
    ...
    "secondary": 3,
    "is_leader": 4,
    # 5 は旧「技能」列。v22 で技能タグ機能ごと廃止したため読まない
    "joined_at": 6,
    "active": 7,
}
...
class Stats:
    ...
    def line(self, name: str) -> str:
        return (
            f"{name}: 入力 {self.input_rows} 行 / 移行 {self.migrated} 件 / "
...
async def _name_to_user_id(db: Database, guild_id: int) -> dict[str, str]:
    ...
    return {r["display_name"]: r["user_id"] for r in rows if counts[r["display_name"]] == 1}
...
# ---------------------------------------------------------------------
async def import_members(db: Database, guild_id: int, rows: list[list], apply: bool) -> Stats:
    """members シート → members テーブル（ユーザーID で重複検知）。

    旧シートの「技能」列は取り込まない（技能タグ機能はスキーマ v22 で廃止）。
    """
    stats = Stats()
    ...
    for i, row in enumerate(rows, start=2):
        ...
        try:
            ...
            if apply:
                ...
                secondary_names = [
                    s.strip() for s in _cell(row, MEM_COLS["secondary"]).split("、") if s.strip()
                ]
                await db.execute(
                    """
                    INSERT INTO members
                        (guild_id, user_id, display_name, primary_team, secondary_teams,
                         is_leader, notes, joined_at, active_flag)
                    VALUES (?, ?, ?, ?, ?, ?, NULL, ?, ?)
                    """,
                    (
                        guild_id,
                        ...
                        json.dumps(
                            [team_map[n] for n in secondary_names if n in team_map],
                            ensure_ascii=False,
                        ),
...
async def import_attendance(db: Database, guild_id: int, rows: list[list], apply: bool) -> Stats:
    ...
    for i, row in enumerate(rows, start=2):
        ...
        try:
            ...
            if not user_id:
                ...
                continue
            exists = await db.fetchone(
                "SELECT 1 FROM schedule_votes WHERE guild_id = ? AND option_id = ? AND user_id = ?",
...
async def import_layer_rows(
    db: Database, guild_id: int, keta: str, rows: list[list], apply: bool
) -> Stats:
...
def _load_sheets() -> dict[str, dict[str, list[list]]]:
    ...
    try:
        ...
        print(
            "ERROR: gspread / google-auth が見つかりません。"
            "移行時のみ次を実行してください:\n"
            "  venv/bin/pip install gspread google-auth"
        )
        sys.exit(1)
    ...
    if not spreadsheet_id or not os.path.exists(creds_path):
        print(
            "ERROR: SPREADSHEET_ID または GOOGLE_CREDENTIALS_PATH（JSON）が"
    ...
    client = gspread.authorize(Credentials.from_service_account_file(creds_path, scopes=scopes))
    ...
    for name in (sheet_attendance, sheet_members):
        try:
    ...
    main_titles = {sheet_attendance, sheet_members}
...
async def main(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        all_stats.append(
            (
                f"members({mem_name})",
                await import_members(db, guild_id, sheets["main"].get(mem_name, []), args.apply),
            )
        )
        all_stats.append(
            (
                f"attendance({att_name})",
                await import_attendance(db, guild_id, sheets["main"].get(att_name, []), args.apply),
            )
        )
        for keta, rows in sheets["layer"].items():
            all_stats.append(
                (f"layer({keta})", await import_layer_rows(db, guild_id, keta, rows, args.apply))
            )
