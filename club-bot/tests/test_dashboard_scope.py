...
async def _seed(db_path: str) -> None:
    ...
    try:
        ...
        await progress.upsert_node(GUILD_A, "a1", name="A大学の機体",
                                   now_text=NOW)
        ...
        for i in range(7):
            await progress.upsert_node(GUILD_B, f"b{i}", name=f"B大学の部品{i}",
                                       now_text=NOW)
    finally:
        await db.close()
...
def test_guild_id_variants_do_not_bypass_check():
    ...
    client = _client_logged_into_a(db_path)
    try:
        for path in (f"/api/guilds/{GUILD_B}/summary",
...
def test_scope_bind_fixes_guild_id():
    ...
    async def _main():
        ...
        await db.connect()
        try:
            scope = GuildScope(guild_id=GUILD_A, guild_name="A大学",
                               user_id=USER_ID, user_name="山田",
                               level=Level.L1, manage_guild=False)
...
def test_scope_require_level():
    scope = GuildScope(guild_id=GUILD_A, guild_name="A大学", user_id=USER_ID,
                       user_name="山田", level=Level.L1, manage_guild=False)
...
def test_resolve_level_from_db():
    ...
    async def _main():
        ...
        await db.connect()
        try:
            assert await security.resolve_level(
                db, GUILD_A, USER_ID, True) == Level.L4
            assert await security.resolve_level(
                db, GUILD_A, USER_ID, False) == Level.L1
            await MemberRepository(db).set_leader(GUILD_A, USER_ID, True)
            assert await security.resolve_level(
                db, GUILD_A, USER_ID, False) == Level.L2
            # 未登録の利用者は既定の L1
            assert await security.resolve_level(
                db, GUILD_A, "999", False) == Level.L1
        finally:
            await db.close()
...
def test_scoped_routes_depend_on_guild_scope():
    ...
    for route in _iter_routes(app.router):
        ...
        scoped = any(hint is GuildScope or GuildScope in get_args(hint)
