...
class Database:
    ...
    _PROGRESS_TABLES = (
        "progress_nodes", "progress_todoist_links", "progress_spar_links",
    )
    ...
    async def _migrate_v8_members_surrogate_pk(self) -> None:
        ...
        try:
            ...
            await self._conn.execute(
                f"INSERT INTO members ({col_list})"
                f" SELECT {col_list} FROM members_legacy")
            ...
            await self._conn.execute("PRAGMA foreign_keys = ON;")
        log.info("members に代理主キー member_id を追加しました（v8, %d 行）。",
